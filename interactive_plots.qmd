...
migrant_stock_changes |>
  group_by(area_of_origin, 
           area_of_destination) |> 
  summarise(migrants_2020 = sum(migrants_2020, na.rm = TRUE))
...
test_df |> select(area_of_origin, area_of_destination, )
```
...
ggnet2(test, color = "sex_ratio", edge.size = "total_migrants_2020")
  ...
  ggplot() + 
  geom_spoke_interactive(aes(radius = total_migrants_2020, 
                             colour = sex_ratio))
...
migration_network_graph <- migrant_stock_changes |> 
  select(area_of_origin, area_of_destination, sex, 
         migrants_2020, origin_population_2020) |> 
  pivot_wider(names_from = "sex", 
              values_from = "migrants_2020", 
