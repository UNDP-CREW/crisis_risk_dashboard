...
maldives |> 
  filter(!is.na(island_correct)) |> 
  count(administration, population)

read_csv(here("data", "maldives_atoll_indicator.csv")) |> 
  left_join(
...
maldives |> 
  ...
  select(
    atoll_official, 
    atoll_local_names = `Atoll Official Name`,
    indicator, 
    value, 
    range, 
    indicator2, 
  ...
  write_csv(here("data", "maldives_atoll_indicator.csv"))
  #write_csv(here("data", "maldives_atoll_indicator2.csv"))

```

```{r}
read_csv(here("data", "maldives_atoll_indicator.csv")) |> glimpse()
  left_join(
    atolls_official |> 
      distinct(atoll_official = Atoll, `Atoll Official Name`), 
    by = "atoll_official"
...
```{r}
maldives  |> 
  filter(population == "Resident Population" & 
           administration == "Administrative & Non-administrative Islands") |>
  select(atoll_official, atoll_local_names, 
         indicator, value) |>
  rbind(maldives_poverty) |> 
  group_by(indicator) |> 
  mutate(range = range_wna(value)) |> 
  ungroup() |> 
  mutate(range2 = range, 
...
maldives |>
  ...
  summarise(population = round(sum(value))) |> 
  left_join(
    atolls_official |> 
      distinct(atoll_official = Atoll, `Atoll Official Name`), 
    by = "atoll_official"
  ...
  write_csv(here("data", "maldives_atoll_populations.csv"))
...
```{r}
maldives |> 
  filter(indicator %in% c("Sex Ratio (males per 100 female)", "Stalking"))
...
islands_atolls |> 
  filter(str_detect(atoll_official, "Kaafu"))
```
