...
```{r}
viridis_pal(option = "plasma")(7)
```


```{r}
viridis_pal(option = "plasma")(7) |> show_col()

viridis_pal(option = "turbo")(7) |> show_col()
```
...
migrant_stock_changes |> 
  ...
  mutate(total_migrants_2020 = male + female, 
         sex_ratio = (round(male / female * 100)),
  ...
  mutate(n = 1,
         origin_population_pc = round(total_migrants_2020 / origin_population_2020 * 100, 
                                      digits = 2)) |> 
  filter(total_migrants_2020 > 100000 | origin_population_pc > 5) |> 
  ...
  geom_edge_link(aes(edge_width = total_migrants_2020, 
                 ...
                 lineend = "round",
                 check_overlap = TRUE) + 
  scale_edge_colour_gradient2(low = "#F0F921FF", mid = "#FDB32FFF", high = "#5D01A6FF", 
                              trans = "log10") +
